...
class Actor(nn.Module):
    ...
    def forward(self, value_msg):
        ...
        x = x.permute(0, 2, 1, 3)  # now (B,K,F,N)
        actions = x.view((batch_size, 1, self.n_a, n_agents))
...
class Critic(nn.Module):
    ...
    def __init__(self, n_s, n_a, msg_len, hidden_layers):
        ...
        self.n_a = n_a
        self.msg_len = msg_len
        self.layers = [n_s + msg_len + msg_len + n_a] + hidden_layers + [1]
        self.n_layers = len(self.layers) - 1
    ...
    def forward(self, value_msg, actions):
        """
        The policy relies on delayed information from neighbors. During training, the full history for k time steps is
        necessary.
        :param value:
        ...
        actions = actions.view((batch_size, 1, self.n_a, n_agents))
        x = torch.cat((value_msg, actions), 2)
        x = x.permute(0, 2, 1, 3)  # now (B,F,K,N)
        ...
        x = x.permute(0, 2, 1, 3)  # now (B,K,F,N), but doesn't do anything sinze K=F=1
